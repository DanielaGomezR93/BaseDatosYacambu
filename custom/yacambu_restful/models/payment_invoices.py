...
class AccountPaymentYacambuRestful(models.Model):
    ...
    def paymnt_invoices(self, partner_id, invoice_id, move_id):
        ...
        for l in lines_moves:
            ...
            _logger.info("este es el monto de la linea DEL PAGO %s",
                         abs(line_account.amount_residual_currency))
            ...
            lines_invoice = self.env['account.move.line'].sudo(
            ).search_read(invoices_domain)
            ...
            for _ in lines_invoice:
                ...
                try:
                    ...
                    Response.status = "400"
                    return {
                        "msg": "Error buscando linea de factura.",
                        "error": e
                    }
